...
$query  = "SELECT t_total.ploNo, t_total.total, t_achieved.achieved
...
WHERE p.programId = '$program'
GROUP BY co.ploNo
ORDER BY co.ploNo) AS t_total
JOIN (SELECT co.ploNo, count(DISTINCT co.studentId) AS achieved
FROM tbl_co AS co
JOIN tbl_course AS c ON co.courseTitle = c.courseTitle
JOIN tbl_program AS p ON c.departmentId = p.departmentId
WHERE p.programId = '$program' AND co.achievedMarks >= 40
GROUP BY co.ploNo
ORDER BY co.ploNo) AS t_achieved ON t_total.ploNo = t_achieved.ploNo";
