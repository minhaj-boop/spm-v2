...
$query  = "SELECT t_total.ploNo, round((t_achieved.achieved*100/t_total.total)) AS perAchieved
FROM (SELECT co.ploNo, count(DISTINCT co.studentId) AS total
FROM tbl_co AS co
JOIN tbl_course AS c ON co.courseTitle = c.courseTitle
JOIN tbl_faculty AS f ON c.facultyId = f.facultyId
WHERE f.facultyName = 'Sadita Ahmed'
GROUP BY co.ploNo
ORDER BY co.ploNo) AS t_total
JOIN (SELECT co1.ploNo, count(DISTINCT co1.studentId) AS achieved 
FROM tbl_co AS co1
JOIN tbl_course AS c ON co1.courseTitle = c.courseTitle
JOIN tbl_faculty AS f ON c.facultyId = f.facultyId
WHERE f.facultyName = 'Sadita Ahmed' AND co1.achievedMarks >= 40
GROUP BY co1.ploNo
